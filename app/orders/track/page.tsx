...
function TrackOrderContent() {
  ...
  useEffect(() => {
    ...
    if (invoice) { setQuery(invoice); search(invoice) }
  }, [])
  ...
  async function search(val?: string) {
    ...
    if (isInvoice)      qb = qb.eq('invoice_number', q)
    else if (isEmail)   qb = qb.eq('buyer_email', q.toLowerCase())
    else                qb = qb.eq('buyer_phone', q)

    const { data: ordersData } = await qb
    ...
    if (ordersData.length === 1) setExpandedId(ordersData[0].id)

    // Fetch order_items for all found orders
    const orderIds = ordersData.map((o: any) => o.id)
    const { data: allItems } = await supabase
      .from('order_items')
      .select('*, artworks(title, sku, preview_url, profiles:artist_id(full_name, display_name))')
      .in('order_id', orderIds)

    // Also fetch legacy single-artwork for orders without items
    ...
    for (const order of ordersData) {
      ...
      if (items.length > 0) {
        ...
        // Legacy order — fetch artwork directly
        const { data: artwork } = await supabase
          .from('artworks')
          .select('title, sku, preview_url, profiles:artist_id(full_name)')
        ...
        {orders.length > 0 && (
          <div>
            ...
            {orders.map(order => {
              ...
              const previewUrl  = firstItem?.artworks?.preview_url
              ...
              return (
                <div key={order.id} className="card" style={{ marginBottom: 12, padding: 0, overflow: 'hidden' }}>
                  ...
                  >
                    {previewUrl && (
                      <img src={previewUrl} alt="" style={{ width: 52, height: 52, objectFit: 'cover', borderRadius: 8, flexShrink: 0, pointerEvents: 'none' }} />
                    )}
                    <div style={{ flex: 1 }}>
                      {items.length === 1 ? (
                        <p style={{ fontSize: 14, fontWeight: 500 }}>
                          {firstItem?.artworks?.title} — {firstItem?.print_size}
                        </p>
                      ) : (
                        <p style={{ fontSize: 14, fontWeight: 500 }}>
                          {items.length} prints
                          <span style={{ fontSize: 12, color: 'var(--color-text-muted)', fontWeight: 400, marginLeft: 6 }}>
                            {items.map((i: any) => i.artworks?.title).join(', ')}
                          </span>
                        </p>
                      )}
                      ...
                      <div style={{ display: 'flex', gap: 8, marginTop: 4, alignItems: 'center' }}>
                        ...
                        <span className="sku-tag" style={{ fontSize: 10 }}>{order.invoice_number}</span>
                      </div>
                    </div>
                  ...
                  {isExpanded && (
                    <div style={{ borderTop: '0.5px solid var(--color-border)', padding: '20px 20px' }}>
                          ...
                          return (
                            <div key={step.key} style={{ display: 'flex', gap: 16, alignItems: 'flex-start', marginBottom: i < STATUS_STEPS.length - 1 ? 20 : 0, position: 'relative' }}>
                              <div style={{
                                ...
                                background: done ? 'var(--color-teal)' : active ? '#1a1a1a' : 'var(--color-surface)',
                      ...
                      {/* Items list */}
                      <p style={{ fontSize: 12, fontWeight: 500, marginBottom: 10, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                        Items · {items.length}
                      </p>
                      {items.map((item: any, i: number) => {
                        ...
                        return (
                          <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'center', padding: '10px 0', borderBottom: '0.5px solid var(--color-border)' }}>
                            {item.artworks?.preview_url && (
                              <img src={item.artworks.preview_url} alt="" style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: 6, pointerEvents: 'none', flexShrink: 0 }} />
                            )}
                            <div style={{ flex: 1 }}>
                              <p style={{ fontSize: 13, fontWeight: 500 }}>{item.artworks?.title}</p>
                              <p style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>
                                by {artistName} · {item.print_size}
                              </p>
                            </div>
                            <p style={{ fontSize: 13, fontWeight: 500, flexShrink: 0 }}>{formatMVR(item.print_price)}</p>
                          </div>
                        )
                      })}
                      ...
                      <div style={{ marginTop: 16, marginBottom: 16 }}>
                        {[
                          ...
                          <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '5px 0', borderBottom: '0.5px solid var(--color-border)' }}>
                            ...
                            <span style={{
                              fontWeight: k === 'Total paid' ? 500 : 400,
                              fontFamily: k === 'Invoice' ? 'var(--font-mono)' : 'inherit',
                              fontSize: k === 'Invoice' ? 11 : 13,
                            }}>{v}</span>
                          </div>
                        ))}
                      </div>
