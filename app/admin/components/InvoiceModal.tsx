...
export function InvoiceModal({ order, onClose }: { order: any; onClose: () => void }) {
  ...
  // Total artist earnings + commission across all items
      ...
      return `
        <tr style="border-bottom:1px solid #f4f4f0">
          ...
          <td style="padding:12px 0;text-align:right;vertical-align:top">
            ${hasOffer ? `<div style="font-size:12px;color:#aaa;text-decoration:line-through">MVR ${item.original_price + item.printing_fee}</div>` : ''}
      ...
      return `
        ...
        ${hasOffer
          ? `<tr><td style="color:#888;padding:3px 0;font-size:12px">Artwork price</td><td style="text-align:right;font-size:12px;color:#aaa;text-decoration:line-through">MVR ${item.original_price}</td></tr>
             <tr><td style="color:#c05030;padding:3px 0;font-size:12px">${item.offer_label} (−${item.offer_pct}%)</td><td style="text-align:right;font-size:12px;color:#c05030">− MVR ${item.discount_amount}</td></tr>`
  ...
  function printInvoice() {
    ...
    .wrap{max-width:620px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e0ddd6}
                ...
                return (
                  <tr key={i} style={{ borderBottom: '1px solid #f4f4f0' }}>
                    ...
                    <td style={{ padding: '12px 0', textAlign: 'right', verticalAlign: 'top' }}>
                      {hasOffer && <p style={{ fontSize: 12, color: '#aaa', textDecoration: 'line-through', margin: 0 }}>MVR {item.original_price + item.printing_fee}</p>}
                ...
                {items.map((item, i) => {
                  ...
                  return (
                    <React.Fragment key={i}>
                      ...
                      {hasOffer ? (
                        <>
                          <tr><td style={{ color: '#888', padding: '3px 0', fontSize: 12 }}>Artwork price</td><td style={{ textAlign: 'right', fontSize: 12, color: '#aaa', textDecoration: 'line-through' }}>MVR {item.original_price}</td></tr>
                          <tr><td style={{ color: '#c05030', padding: '3px 0', fontSize: 12 }}>{item.offer_label} (−{item.offer_pct}%)</td><td style={{ textAlign: 'right', fontSize: 12, color: '#c05030' }}>− MVR {item.discount_amount}</td></tr>
                        </>
                      ) : (
                        <tr><td style={{ color: '#888', padding: '3px 0', fontSize: 12 }}>Artwork price</td><td style={{ textAlign: 'right', fontSize: 12 }}>MVR {item.original_price}</td></tr>
                      )}
                      <tr><td style={{ color: '#888', padding: '3px 0', fontSize: 12 }}>{item.print_size} giclée printing</td><td style={{ textAlign: 'right', fontSize: 12 }}>MVR {printingFee}</td></tr>
                    </React.Fragment>
                  )
                })}
                {order.delivery_method === 'delivery' ? (
