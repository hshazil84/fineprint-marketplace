...
\
  async function updateStatus(newStatus: string) \{\
    ...
    if (data.success) \{\
      toast.success(shouldSendEmail ? 'Status updated \'97 buyer notified!' : 'Status updated')\
      onStatusChange()\
    \} else \{\
    ...
    <div style=\{\{ padding: '16px 20px', borderBottom: '0.5px solid var(--color-border)' \}\}>\
      <div style=\{\{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 10 \}\}>\
        <div style=\{\{ flex: 1 \}\}>\
          ...
          <p style=\{\{ fontSize: 12, color: 'var(--color-text-muted)', marginTop: 2 \}\}>\
            \{order.buyer_name\} \'b7 \{new Date(order.created_at).toLocaleDateString()\} \'b7 \{formatMVR(order.total_paid)\}\
            \{' \'b7 '\}\{order.delivery_method === 'pickup' ? '\uc0\u55356 \u57322  Pickup' : `\u55357 \u56550  Deliver \u8594  $\{order.delivery_island\}`\}\
          </p>\
        </div>\
      </div>\
        ...
        <div style=\{\{ display: 'flex', gap: 6, marginLeft: 'auto' \}\}>\
          \{order.slip_url && (\
            <button className="btn btn-sm"\
              ...
              onClick=\{onViewSlip\}>\uc0\u55357 \u56526  Slip</button>\
          )\}\
          \{order.status !== 'pending' && order.status !== 'rejected' && (\
            <button className="btn btn-sm" style=\{\{ fontSize: 11, padding: '3px 10px' \}\} onClick=\{onViewInvoice\}>\uc0\u55357 \u56516  Invoice</button>\
          )\}\
      ...
      \{order.status === 'ready' && order.delivery_method === 'pickup' && (\
        <p style=\{\{ fontSize: 11, color: 'var(--color-teal-dark)', marginTop: 8, background: 'var(--color-teal-light)', padding: '4px 10px', borderRadius: 6, display: 'inline-block' \}\}>\
          \uc0\u55357 \u56542  Pickup email tells buyer to call 9998124 to arrange collection\
        </p>\
      )\}\
      \{order.status === 'ready' && order.delivery_method === 'delivery' && (\
        <p style=\{\{ fontSize: 11, color: 'var(--color-teal-dark)', marginTop: 8, background: 'var(--color-teal-light)', padding: '4px 10px', borderRadius: 6, display: 'inline-block' \}\}>\
          \uc0\u55357 \u56550  Delivery email tells buyer to expect a call from 9998124\
        </p>\
      )\}\
    </div>\
  )\
      ...
      <div className="grid-2" style=\{\{ marginBottom: 12 \}\}>\
        <div className="form-group">\
          ...
          <select className="form-input" value=\{artist\} onChange=\{e => setArtist(e.target.value)\}>\
            ...
            \{artists.map((a: any) => (\
              <option key=\{a.id\} value=\{a.artist_code\}>FP-\{a.artist_code\} \'97 \{a.display_name || a.full_name\}</option>\
            ))\}\
          </select>\
        </div>\
        <div className="form-group">\
          ...
          <select className="form-input" onChange=\{e => applyMonth(e.target.value)\} defaultValue="">\
            <option value="">\'97 pick a month \'97</option>\
...
\
  async function handleOrderAction(invoiceNumber: string, action: 'approve' | 'reject') \{\
    ...
    const data = await res.json()\
    if (data.success) \{\
      toast.success(action === 'approve' ? 'Order approved \'97 invoice sent!' : 'Order rejected')\
      fetchOrders()\
    \} else \{\
      toast.error(data.error)\
    \}\
...
\
        \{withdrawRequests.length > 0 && (\
          <div style=\{\{ background: '#FCEBEB', border: '0.5px solid #F09595', borderRadius: 8, padding: '10px 16px', marginBottom: 16, fontSize: 13, color: '#A32D2D' \}\}>\
            \uc0\u9888 \u65039  \{withdrawRequests.length\} artist\{withdrawRequests.length > 1 ? 's have' : ' has'\} requested to withdraw. Check the Artists tab.\
          </div>\
        )\}\
        \{closedShops.length > 0 && (\
          <div style=\{\{ background: '#FAEEDA', border: '0.5px solid #EF9F27', borderRadius: 8, padding: '10px 16px', marginBottom: 16, fontSize: 13, color: '#633806' \}\}>\
            \uc0\u55357 \u56594  \{closedShops.length\} artist\{closedShops.length > 1 ? 's have' : ' has'\} temporarily closed their shop.\
          </div>\
        )\}\
\
            ...
            \{pendingPayouts.length > 0 && (\
              <div style=\{\{ marginBottom: 24 \}\}>\
                <p style=\{\{ fontSize: 14, fontWeight: 500, marginBottom: 12 \}\}>\
                  \uc0\u55357 \u56504  Pending payout requests\
                  ...
                  \{pendingPayouts.map(p => (\
                    <div key=\{p.id\} style=\{\{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '0.5px solid var(--color-border)', gap: 12 \}\}>\
                      <div>\
                        ...
                        <p style=\{\{ fontSize: 13, color: 'var(--color-text-muted)', marginTop: 2 \}\}>\{p.bank_name\} \'b7 \{p.account_name\}</p>\
                        <div style=\{\{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 \}\}>\
                          ...
                          <button className="btn btn-sm" style=\{\{ fontSize: 11, padding: '2px 8px' \}\}\
                            onClick=\{() => \{ navigator.clipboard.writeText(p.account_number); toast.success('Copied!') \}\}>Copy</button>\
                        </div>\
...
\
            \{withdrawRequests.length > 0 && (\
              <div style=\{\{ marginBottom: 24 \}\}>\
                <p style=\{\{ fontSize: 14, fontWeight: 500, marginBottom: 12, color: '#A32D2D' \}\}>\uc0\u9888 \u65039  Withdrawal requests</p>\
                <div className="card" style=\{\{ padding: 0, overflow: 'hidden' \}\}>\
                  ...
                  \{paidPayouts.map(p => (\
                    <div key=\{p.id\} style=\{\{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: '0.5px solid var(--color-border)', gap: 12 \}\}>\
                      <div>\
                        ...
                        <p style=\{\{ fontSize: 12, color: 'var(--color-text-muted)', marginTop: 2 \}\}>\{p.bank_name\} \'b7 \{p.account_number\}</p>\
                        <p style=\{\{ fontSize: 11, color: 'var(--color-text-muted)', marginTop: 2 \}\}>\{p.paid_at ? `Paid $\{new Date(p.paid_at).toLocaleDateString()\}` : ''\}</p>\
                        <button className="btn btn-sm" style=\{\{ marginTop: 6, fontSize: 11 \}\} onClick=\{() => setRemittancePayout(p)\}>\uc0\u55357 \u56516  View remittance</button>\
                      </div>\
                ...
                return (\
                  <div key=\{a.id\} style=\{\{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '0.5px solid var(--color-border)' \}\}>\
                    <div>\
                      ...
                      <p style=\{\{ fontSize: 12, color: 'var(--color-text-muted)', marginTop: 2 \}\}>\
                        FP-\{a.artist_code\} \'b7 \{a.email\} \'b7 \{artworkCount\} listings \'b7 \{artistOrders.length\} sales\
                      </p>\
                    </div>\
                    <div style=\{\{ textAlign: 'right' \}\}>\
              ...
              <div key=\{a.id\} style=\{\{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: '0.5px solid var(--color-border)', gap: 12 \}\}>\
                ...
                <div style=\{\{ flex: 1 \}\}>\
                  ...
                  <p style=\{\{ fontSize: 12, color: 'var(--color-text-muted)' \}\}>\
                    by \{a.profiles?.display_name || a.profiles?.full_name\} \'b7 \{formatMVR(a.price)\}\
                    \{a.offer_label ? ` \'b7 $\{a.offer_label\} \uc0\u8722 $\{a.offer_pct\}%` : ''\}\
                  </p>\
                </div>\
                <div style=\{\{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center' \}\}>\
                  \{a.hires_path && (\
                    <button className="btn btn-sm" onClick=\{() => downloadHires(a.hires_path)\}>\uc0\u11015  Hi-res</button>\
                  )\}\
                  \{a.status === 'pending' && (\
                    <>\
          ...
          <div className="card" style=\{\{ padding: 0, overflow: 'hidden' \}\}>\
            ...
            \{artworks.filter(a => a.offer_pct).length === 0 ? (\
              ...
              <div key=\{a.id\} style=\{\{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: '0.5px solid var(--color-border)' \}\}>\
                <div>\
                  <p style=\{\{ fontSize: 14, fontWeight: 500 \}\}>\{a.offer_label\} \'97 \{a.profiles?.display_name || a.profiles?.full_name\}</p>\
                  <p style=\{\{ fontSize: 12, color: 'var(--color-text-muted)', marginTop: 2 \}\}>\
                    \{a.sku\} \'b7 \{a.offer_pct\}% off\{a.offer_expires ? ` \'b7 Expires $\{a.offer_expires\}` : ' \'b7 No expiry'\}\
                  </p>\
                </div>\
