    ...
    <div style={{ background: 'var(--color-background-primary)', minHeight: '100vh' }}>
      <style>{`
        ...
        @media(max-width: 768px) {
          ...
          .mobile-carousel-wrap { display: block !important; }
          .mobile-grid { display: grid !important; }
        }
        @media(min-width: 769px) {
          .mobile-only { display: none !important; }
          .desktop-only { display: flex !important; }
          .mobile-carousel-wrap { display: none !important; }
      ...
      <nav style={{
        ...
        <div style={{ maxWidth: 1080, margin: '0 auto', padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          {/* Desktop nav */}
          <div className="desktop-only" style={{ flex: 1, alignItems: 'center', gap: 12 }}>
            <div style={{ flex: 1, maxWidth: 360, position: 'relative' }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search artworks, artists..."
              ...
              {search && (
                <button onClick={() => setSearch('')} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, color: 'var(--color-text-muted)' }}>✕</button>
              )}
            </div>
          ...
          {/* Mobile hamburger */}
          <div className="mobile-only" style={{ marginLeft: 'auto', alignItems: 'center' }}>
            <button onClick={() => setMenuOpen(!menuOpen)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 8, color: 'var(--color-text)' }}>
              {menuOpen ? (
        ...
        {/* Mobile menu drawer */}
        {menuOpen && (
          <div style={{ background: 'var(--color-background-primary)', borderTop: '0.5px solid var(--color-border)', padding: '16px 24px 24px', animation: 'slideDown 0.2s ease' }}>
            {/* Search */}
            <div style={{ position: 'relative', marginBottom: 16 }}>
              <svg style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', width: 13, height: 13, opacity: 0.4 }} viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
                <circle cx="6.5" cy="6.5" r="4.5" /><path d="M10.5 10.5 14 14" />
              </svg>
              <input value={search} onChange={e => { setSearch(e.target.value); setMenuOpen(false) }} placeholder="Search artworks, artists..."
                style={{ width: '100%', padding: '10px 14px 10px 34px', background: 'var(--color-background-secondary)', border: '0.5px solid var(--color-border)', borderRadius: 20, fontSize: 14, color: 'var(--color-text)', outline: 'none' }} />
            </div>
            {/* Links */}
            {[
              ...
              <Link key={href} href={href} onClick={() => setMenuOpen(false)}
                style={{ display: 'block', fontSize: 15, color: 'var(--color-text)', textDecoration: 'none', padding: '12px 0', borderBottom: '0.5px solid var(--color-border)' }}>
                {label}
              </Link>
            ))}
          </div>
      ...
      {/* ARTIST TICKER */}
      {artists.length > 0 && (
        <div style={{ borderBottom: '0.5px solid var(--color-border)', overflow: 'hidden', height: 68, display: 'flex', alignItems: 'center', position: 'relative' }}>
          {/* Feathered edges */}
          <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 80, background: 'linear-gradient(to right, var(--color-background-primary), transparent)', zIndex: 10, pointerEvents: 'none' }} />
          <div style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: 80, background: 'linear-gradient(to left, var(--color-background-primary), transparent)', zIndex: 10, pointerEvents: 'none' }} />
          <div style={{ maxWidth: 1080, margin: '0 auto', width: '100%', overflow: 'hidden', position: 'relative' }}>
            <div className="ticker-scroll" style={{ display: 'flex', gap: 14, width: 'max-content', padding: '0 40px' }}>
              {tickerArtists.map((artist, i) => {
                const color = getColor(artist.artist_code || 'FP')
                return (
                  <div key={`${artist.id}-${i}`} style={{ width: 48, height: 48, borderRadius: '50%', flexShrink: 0, overflow: 'hidden', cursor: 'pointer', border: '2px solid var(--color-border)', transition: 'transform 0.2s, border-color 0.2s' }}
                    onMouseEnter={e => { e.currentTarget.style.transform = 'scale(1.1)'; e.currentTarget.style.borderColor = color }}
                    ...
                    {artist.avatar_url ? (
                      ...
                      <div style={{ width: '100%', height: '100%', background: color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 500, color: '#fff' }}>
          ...
          <>
            ...
            {!search && activeCategory === 'All' && recentSix.length > 0 && (
              <div style={{ marginBottom: 40 }}>
                ...
                <div className="mobile-carousel-wrap" style={{ display: 'none' }}>
                  <div style={{ display: 'flex', gap: 12, overflowX: 'auto', scrollSnapType: 'x mandatory', WebkitOverflowScrolling: 'touch' as any, scrollbarWidth: 'none', paddingBottom: 4 }}>
                    {recentSix.map(artwork => (
                      <div key={artwork.id} style={{ flex: '0 0 78%', scrollSnapAlign: 'start' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 16 }}>
              ...
              {(search || activeCategory !== 'All') && (
                <button onClick={() => { setSearch(''); setActiveCategory('All') }} style={{ fontSize: 12, color: '#1D9E75', background: 'none', border: 'none', cursor: 'pointer' }}>Clear ✕</button>
              )}
            </div>
