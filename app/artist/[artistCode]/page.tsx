// app/artist/[artistCode]/page.tsx
// Server component — instant load, no skeleton
// Only ArtistNotifyButton is client-side (waitlist email input)
...
import ArtistNotifyButton from './ArtistNotifyButton'
...
export default async function ArtistProfilePage({
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('*')
    .eq('artist_code', params.artistCode)
    .eq('role', 'artist')
    .single()
  ...
  // Compute top 5 sellers server-side
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 24, flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 6 }}>
                <h1 style={{ fontSize: '1.4rem', fontWeight: 500, fontFamily: 'var(--font-display)' }}>
                  {displayName}
                </h1>
            ...
            <div style={{ textAlign: 'center', flexShrink: 0 }}>
              <p style={{ fontSize: 22, fontWeight: 500, fontFamily: 'var(--font-display)' }}>
                {artworks.length}
              </p>
...
function ArtworkCard({
  ...
  const fromPrice = artwork.price + Math.min(...sizes.map((s: string) => {
    return PRINTING_FEES[s] || PRINTING_FEES['A4']
  }))
    ...
    <Link href={'/artwork/' + artwork.id} style={{ textDecoration: 'none' }}>
      <div style={{ borderRadius: 12, overflow: 'hidden', border: '0.5px solid var(--color-border)', backgroundColor: 'var(--color-background-primary)', transition: 'border-color 0.15s' }}>
        ...
        <div style={{ padding: '10px 12px 12px' }}>
          ...
          {/* Notify button — only interactive piece */}
          {isSoldOut && (
            <div onClick={e => e.preventDefault()}>
              <ArtistNotifyButton artworkId={artwork.id} />
            </div>
          )}
        </div>
      </div>
