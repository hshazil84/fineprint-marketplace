            ...
            return (
              <div key={a.id} style={{ background: 'var(--color-background-primary)', border: '0.5px solid var(--color-border)', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ padding: '6px 10px 10px', display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  ...
                  {a.status !== 'pending' && (
                    <button className="btn btn-sm" style={{ fontSize: 10, padding: '3px 8px', background: a.status === 'hidden' ? 'var(--color-teal-light)' : 'transparent', color: a.status === 'hidden' ? 'var(--color-teal-dark)' : 'var(--color-text-muted)', border: a.status === 'hidden' ? 'none' : '0.5px solid var(--color-border)' }} onClick={() => onToggleHide(a)}>
            ...
            return (
              <div key={a.id} style={{ borderBottom: '0.5px solid var(--color-border)' }}>
                ...
                <div style={{ display: 'flex', gap: 8, padding: '0 20px 14px', flexWrap: 'wrap' }}>
                  ...
                  {a.status !== 'pending' && (
                    <button className="btn btn-sm" style={{ fontSize: 11, background: a.status === 'hidden' ? 'var(--color-teal-light)' : 'var(--color-background-secondary)', color: a.status === 'hidden' ? 'var(--color-teal-dark)' : 'var(--color-text-muted)', border: 'none' }} onClick={() => onToggleHide(a)}>
