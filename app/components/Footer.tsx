...
export default function Footer() {
  return (
    <footer style={{ borderTop: '0.5px solid var(--color-border)', background: 'var(--color-background-secondary)' }}>
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '36px 40px 24px' }}>

        {/* Desktop: full footer */}
        <div className="footer-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 40, marginBottom: 28 }}>
          <div className="footer-brand">
            <div style={{ marginBottom: 10 }} dangerouslySetInnerHTML={{ __html: LOGO_SVG }} />
            <p style={{ fontSize: 12, color: 'var(--color-text-muted)', lineHeight: 1.7, marginBottom: 14 }}>
            ...
            <Image
              ...
              width={120}
              height={40}
              style={{ width: 120, height: 'auto', opacity: 0.85 }}
            />
          </div>
          <div>
            <p style={{ fontSize: 10, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--color-text-muted)', marginBottom: 12 }}>Shop</p>
            {[['Browse prints', '/storefront'], ['Track your order', '/orders/track'], ['Artist signup', '/auth/signup']].map(([label, href]) => (
              <Link key={href} href={href} style={{ display: 'block', fontSize: 13, color: 'var(--color-text-muted)', textDecoration: 'none', marginBottom: 7 }}>{label}</Link>
            ))}
          </div>
          <div>
            <p style={{ fontSize: 10, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--color-text-muted)', marginBottom: 12 }}>Legal</p>
            {[['Terms & conditions', '/terms'], ['Privacy policy', '/privacy'], ['Cookie policy', '/cookies']].map(([label, href]) => (
              <Link key={href} href={href} style={{ display: 'block', fontSize: 13, color: 'var(--color-text-muted)', textDecoration: 'none', marginBottom: 7 }}>{label}</Link>
            ))}
          </div>
          <div>
            <p style={{ fontSize: 10, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--color-text-muted)', marginBottom: 12 }}>Contact</p>
            <a href="mailto:hello@fineprintmv.com" style={{ display: 'block', fontSize: 13, color: 'var(--color-text-muted)', textDecoration: 'none', marginBottom: 7 }}>hello@fineprintmv.com</a>
            <a href="tel:9998124" style={{ display: 'block', fontSize: 13, color: 'var(--color-text-muted)', textDecoration: 'none', marginBottom: 7 }}>9998124</a>
      ...
      <style>{`@media(max-width:768px){.footer-hint{display:none!important;}}`}</style>
    </footer>
  )
}
