...
export default function Header({
  ...
  return (
    <>
      ...
      <nav
        className="fp-nav"
        style={{ borderBottom: '0.5px solid var(--color-border)', position: 'sticky', top: 0, zIndex: 100 }}
      >
              ...
              <div className="fp-header-desktop" style={{ flex: 1, alignItems: 'center', gap: 12 }}>
                {onSearchChange && (
                  <div style={{ flex: 1, maxWidth: 360, position: 'relative' }}>
                    <svg
                      style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', width: 13, height: 13, opacity: 0.4 }}
                    ...
                    {search && (
                      <button
                        ...
                        style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, color: 'var(--color-text-muted)' }}
                      >
                        x
                      </button>
                    )}
                ...
                <div style={{ display: 'flex', gap: 8, marginLeft: 'auto', alignItems: 'center' }}>
                  <Link
                    href="/orders/track"
                    style={{ fontSize: 12, padding: '6px 14px', borderRadius: 20, border: '0.5px solid var(--color-border)', color: 'var(--color-text)', textDecoration: 'none', whiteSpace: 'nowrap' }}
                  >
                    Track order
                  </Link>
                  ...
                  {user && profile ? (
                    <>
                      ...
                      <button
                        ...
                        style={{ fontSize: 12, padding: '6px 14px', borderRadius: 20, border: '0.5px solid var(--color-border)', background: 'none', color: 'var(--color-text)', cursor: 'pointer', whiteSpace: 'nowrap' }}
                      >
                        Log out
                      </button>
                    </>
                    ...
                    <>
                      <Link
                        href="/auth/login"
                        style={{ fontSize: 12, padding: '6px 14px', borderRadius: 20, border: '0.5px solid var(--color-border)', color: 'var(--color-text)', textDecoration: 'none' }}
                      >
                        Log in
                      </Link>
                      <Link
                        href="/auth/signup"
                        style={{ fontSize: 12, padding: '6px 14px', borderRadius: 20, border: '0.5px solid #1a1a1a', background: '#1a1a1a', color: '#fff', textDecoration: 'none' }}
                      >
                        Sign up
                      </Link>
                    </>
        ...
        {!minimal && menuOpen && (
          <div
            className="fp-nav"
            style={{ borderTop: '0.5px solid var(--color-border)', padding: '16px 24px 24px', animation: 'fp-slide-down 0.18s ease' }}
          >
            <form onSubmit={handleMobileSearchSubmit} style={{ marginBottom: 16 }}>
              <div style={{ position: 'relative' }}>
                <svg
                  style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', width: 14, height: 14, opacity: 0.4 }}
                  viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5"
                >
                  <circle cx="6.5" cy="6.5" r="4.5" />
                  <path d="M10.5 10.5 14 14" />
            ...
            {user && profile ? (
              <>
                ...
                <button
                  onClick={handleSignOut}
                  style={{ display: 'block', width: '100%', textAlign: 'left', fontSize: 15, color: 'var(--color-text)', background: 'none', border: 'none', cursor: 'pointer', padding: '13px 0', borderBottom: '0.5px solid var(--color-border)' }}
                >
                  Log out
                </button>
              </>
