    ...
    async function loadUser() {
      ...
      if (user) {
        ...
        const { data: prof } = await supabase
          .from('profiles')
          .select('full_name, display_name, role, avatar_url, avatar_config')
          .eq('id', user.id)
          .single()
        setProfile(prof)
      }
                ...
                <div style={{ display: 'flex', gap: 6, marginLeft: 'auto', alignItems: 'center' }}>
                  ...
                  {user && profile ? (
                    <div ref={avatarRef} style={{ position: 'relative' }}>
                      {/* Avatar button — uses AvatarDisplay */}
                      <button
                      ...
                      <AnimatePresence>
                        {avatarOpen && (
                          ...
                          >
                            {/* Name + role header */}
                            <div style={{ padding: '12px 16px 10px', borderBottom: '0.5px solid var(--color-border)', display: 'flex', alignItems: 'center', gap: 10 }}>
                              <AvatarDisplay profile={profile} size={32} />
                              <div>
              ...
              <form onSubmit={handleMobileSearchSubmit} style={{ padding: '12px 20px 14px' }}>
                <div style={{ position: 'relative' }}>
                  ...
                  <input
                    ref={mobileSearchRef}
                    value={mobileSearch}
                    onChange={e => { setMobileSearch(e.target.value); if (onSearchChange) onSearchChange(e.target.value) }}
                  ...
                  <button type="submit" style={{ position: 'absolute', right: 6, top: '50%', transform: 'translateY(-50%)', background: '#1a1a1a', border: 'none', cursor: 'pointer', borderRadius: 18, padding: '6px 14px', fontSize: 13, color: '#fff', fontWeight: 500 }}>Go</button>
                </div>
              </form>
            </motion.div>
          )}
            ...
            <div style={{ padding: '28px 28px 48px' }}>
              <nav>
                {navLinks
                  ...
                  .map((link, i) => (
                    <motion.div key={link.href} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.055, duration: 0.26, ease: 'easeOut' }}>
                      <Link
                        href={link.href}
...
function MobileIconBtn({ children, label, onClick }: { children: ReactNode; label: string; onClick: () => void }) {
  return (
    <button aria-label={label} onClick={onClick} style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', width: 40, height: 40, borderRadius: 8, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text)', WebkitTapHighlightColor: 'transparent' }}>
      {children}
    </button>
  )
}
...
function SearchIcon() {
  return <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg>
}

function CartIcon() {
  return <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z" /><line x1="3" y1="6" x2="21" y2="6" /><path d="M16 10a4 4 0 0 1-8 0" /></svg>
}

function DashboardIcon() {
  return <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" /><rect x="3" y="14" width="7" height="7" /><rect x="14" y="14" width="7" height="7" /></svg>
}

function LogOutIcon() {
  return <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" /><polyline points="16 17 21 12 16 7" /><line x1="21" y1="12" x2="9" y2="12" /></svg>
}

function BurgerIcon() {
  return <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"><line x1="3" y1="6" x2="21" y2="6" /><line x1="3" y1="12" x2="21" y2="12" /><line x1="3" y1="18" x2="21" y2="18" /></svg>
}

function CloseIcon() {
  return <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
}
